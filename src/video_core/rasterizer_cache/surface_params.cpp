...
bool SurfaceParams::ExactMatch(const SurfaceParams& other_surface) const {
    return std::tie(other_surface.addr, other_surface.width, other_surface.height,
                    other_surface.stride, other_surface.levels, other_surface.pixel_format,
                    other_surface.is_tiled) ==
               std::tie(addr, width, height, stride, levels, pixel_format, is_tiled) &&
...
Common::Rectangle<u32> SurfaceParams::GetSubRect(const SurfaceParams& sub_surface) const {
    ...
    if (is_tiled) {
        ...
        // Top to bottom
        return {x0, height - y0, x0 + sub_surface.width, height - (y0 + sub_surface.height)};
    }
